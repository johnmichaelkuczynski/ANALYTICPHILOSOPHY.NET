...
- User has flip-flopped on auth repeatedly (3 rip-outs, 3 rebuilds as of July 2026). Current state after rebuild #3 (July 2026): canonical Passport-based auth installed verbatim from the user's attached server/auth.ts, and the site is LOGIN-REQUIRED ("no Google sign-in → no site, PERIOD"): all pages except landing redirect to /, all /api/* return 401 anonymously except bare GET /api (health) and /api/auth/*. Admin `/api/admin/visits` + the Administrative sidebar item are owner-only (johnmichaelkuczynski@gmail.com). Never re-add or remove auth without an explicit instruction, and always update the replit.md auth section to match the current state.
