...
- User has flip-flopped on auth repeatedly (3 rip-outs, 3 rebuilds, then an August 2026 reversal of the lockdown). Current state (Aug 2026, explicit directive "LET THE USER SEE THE APP"): site is OPEN to anonymous visitors; AI endpoints meter output per session (~1,400 chars free sample in api-server lib/quota.ts) then return 401 code LOGIN_REQUIRED, which the frontend turns into a Google sign-in dialog. Owner-only (the ADMIN_EMAIL owner account via isAdmin): /api/admin/*, /api/logins, /api/diagnostics/*. Never tighten or loosen auth without an explicit instruction, and always update replit.md's auth section to match.
