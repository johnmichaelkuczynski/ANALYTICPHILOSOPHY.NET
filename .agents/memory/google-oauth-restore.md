...
- User has flip-flopped on auth repeatedly (3 rip-outs, 3 rebuilds as of July 2026). Current state after rebuild #3: canonical Passport-based auth installed verbatim from the user's attached server/auth.ts, login OPTIONAL (app fully open), admin `/api/admin/visits` gated to johnmichaelkuczynski@gmail.com. Never re-add or remove auth without an explicit instruction, and always update the replit.md auth section to match the current state.
