...
async function main() {
  ...
  // Delete existing Schopenhauer embeddings only
  console.log("🗑️  Clearing existing Schopenhauer embeddings...");
  await db.delete(paperChunks).where(eq(paperChunks.figureId, 'schopenhauer'));
  console.log("✓ Cleared\n");
  
  const startTime = Date.now();
  
  try {
    console.log(`📄 Processing: Essays of Schopenhauer`);
    
    const content = readFileSync(join(__dirname, "schopenhauer_essays.txt"), "utf-8");
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      await db.insert(paperChunks).values({
        ...
        paperTitle: "Essays of Schopenhauer",
        content: chunk,
    ...
    console.log(`\n🎉 Done! Generated ${chunks.length} Schopenhauer embeddings in ${duration} minutes.`);
  } catch (error) {
