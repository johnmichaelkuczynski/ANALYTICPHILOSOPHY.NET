...
async function main() {
  try {
    console.log('Reading Hume\'s "A Treatise of Human Nature"...');
    const text = readFileSync('hume_treatise.txt', 'utf-8');
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      await db.insert(paperChunks).values({
        figureId: FIGURE_ID,
        paperTitle: "A Treatise of Human Nature",
        chunkIndex: i,
        content: chunk,
        embedding: embedding,
      });
