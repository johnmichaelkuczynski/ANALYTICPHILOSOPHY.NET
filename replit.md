...
**Authentication: direct Google OAuth 2.0, enforced in production only.** The server implements the authorization-code flow itself (`/api/auth/google` → Google consent → `/api/auth/google/callback` → token exchange → userinfo → user upsert + login-event insert → session regenerate), with cookie sessions via `express-session` + `connect-pg-simple` (Postgres `session` table, `sid` cookie, httpOnly, sameSite=lax, secure in prod). Requires `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, and `SESSION_SECRET`. Both callback URLs (dev preview domain and published domain, each `https://<domain>/api/auth/google/callback`) must be registered as Authorized redirect URIs on the Google client. Optional `APP_BASE_URL` pins the callback base instead of deriving it from the Host header. Enforcement is production-only: `requireAuth` mounts on `/api` only when `NODE_ENV === "production"` (`/api/healthz` stays public); the dev preview stays open because the preview iframe drops the sameSite cookie. The Administrative tab and `/api/admin/*` plus `/api/diagnostics/*` in production are owner-only (johnmichaelkuczynski@gmail.com, server-side 403). The Administrative page lists every visitor by Gmail with timestamps, plus all-time / 24-hour / month / year totals and a graph per timeframe. The system diagnostic includes auth self-tests: secret presence, live Google credential validation, kickoff redirect check, session-store round-trip, and a synthetic login exercising the same upsert/login-event path as the real callback (cleaned up afterwards). Note: the data model has no per-user scoping — any signed-in account sees the same single-user course data; login is a gate, not multi-tenancy.
