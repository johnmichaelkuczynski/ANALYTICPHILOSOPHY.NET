...
**Authentication: direct Google OAuth 2.0 (rebuilt July 2026 at explicit user request after two prior rip-outs), enforced in production only.** The server implements the authorization-code flow itself (`/api/auth/google` → Google consent → `/api/auth/google/callback` → token exchange → userinfo → user upsert + login-event insert → session regenerate), with cookie sessions via `express-session` + `connect-pg-simple`. Requires `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, and `SESSION_SECRET` (these exact names — NOT the old `GOOGLE_OAUTH_*` names). Both callback URLs (dev preview domain and published domain, each `https://<domain>/api/auth/google/callback`) must be registered as Authorized redirect URIs on the Google client. Enforcement is production-only: `requireAuth` mounts on `/api` only when `NODE_ENV === "production"` (`/api/healthz` and bare `GET /api` stay public — the latter is required by deployment health checks). The landing page has exactly ONE "Sign in with Google" button (hero only — the user explicitly complained about a second header button; do not re-add it). The Administrative tab and `/api/admin/*` + `/api/diagnostics/*` in production are owner-only (johnmichaelkuczynski@gmail.com). The Administrative page lists visitors by Gmail with timestamps plus all-time / 24-hour / month / year totals and graphs. The system diagnostic includes auth self-tests (secret presence, live credential validation, kickoff redirect, session round-trip, synthetic login).
