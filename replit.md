...
**Authentication: NONE — removed by explicit user directive (July 2026, THIRD and final rip-out; the user also deleted the Google keys from the project themselves).** There is no OAuth, no sessions, no `express-session`/`connect-pg-simple`, no auth middleware, and no route gating anywhere — every page and every `/api/*` endpoint is open in both development and production. The landing page uses plain "Start the course" / "Enter the course" buttons that route straight to the dashboard. The `users` and `login_events` tables and the `/logins` + `/admin/visitors` endpoints remain (harmless, empty) so the Administrative page still renders. No Google or session secrets exist in the project anymore. `GET /api` (bare path) is a public health endpoint required by deployment health checks — keep it. **Do not re-add any authentication unless the user explicitly asks for it.**
