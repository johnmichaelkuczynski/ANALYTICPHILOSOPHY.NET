...
**Authentication: Clerk Google login, enforced in production only.** The published site requires signing in (Google or email via Clerk); the dev preview stays open because Clerk's session cookie is dropped inside the Replit preview iframe (treated as third-party), which would 401 every `/api` call in dev. The Clerk wiring itself (provider, proxy middleware, `clerkMiddleware`, `/sign-in` + `/sign-up` routes) is canonical and unconditional; only *enforcement* is gated: the backend mounts `requireAuth` on `/api` only when `NODE_ENV === "production"` (`/api/healthz` stays public for health checks), and the frontend route guards (`protect()`, `HomeRedirect`) activate only in production builds. Requires `CLERK_SECRET_KEY`, `CLERK_PUBLISHABLE_KEY`, and `VITE_CLERK_PUBLISHABLE_KEY` (auto-provisioned). Note: the data model has no per-user scoping — any signed-in account sees the same single-user course data; login is a gate, not multi-tenancy.
