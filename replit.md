...
**Authentication: REQUIRED Google login via the user's canonical Passport auth.ts (July 2026 rebuild #3, locked down by explicit user directive: "if users do not sign in with Google, they do not see the site. PERIOD").** Logged-out visitors see a bare sign-in gate — the course title and a single "Sign in with Google" button, with NO course descriptions, feature cards, or marketing content; every other page redirects to `/`, and every course `/api/*` endpoint returns 401 without a session. Anonymous exceptions: bare `GET /api` (deployment health checks) and `/api/auth/*` (the login flow itself). The Administrative sidebar item renders only for the signed-in owner. `artifacts/api-server/src/auth.ts` is the user's canonical Passport + Google OAuth + `express-session`/`connect-pg-simple` module — **do not rewrite its logic**; only app-specific values (callback path under `/api`, Replit domains, port, DB SSL) were adapted. Secret name chain: `GOOGLE_LOGIN_CLIENT_ID/SECRET` → `GOOGLE_OAUTH_*` → `GOOGLE_*` (the live keys are the older names) plus `SESSION_SECRET` (required in production). Routes: `GET /api/auth/google` (login), `GET /api/auth/google/callback`, `GET /api/auth/user`, `GET /api/auth/me`, `POST /api/auth/logout`, and `GET /api/admin/visits` — owner-only, restricted to `johnmichaelkuczynski@gmail.com`. Sessions live in the `user_sessions` table (declared in the drizzle schema because connect-pg-simple's `createTableIfMissing` can't find its SQL file inside the esbuild bundle). Landing page has exactly ONE "Sign in with Google" button (hero); the TopBar shows signed-in identity + Sign out. `GET /api` (bare path) is a public health endpoint required by deployment health checks — keep it. **Do not remove or re-add authentication unless the user explicitly asks.**
