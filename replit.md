...
- **NEW: Kuczynski Training Expanded** (Nov 13): Added "Russell's Improvements on Frege's Work (Chapter 6)" to knowledge base - 31 new chunks on Theory of Descriptions, definite descriptions, belief reports, identity statements, and critiques of Frege's semantics. Kuczynski now draws from 87 distinct sources (~3,500+ chunks) covering logic, epistemology, psychopathology, AI, ethics, and analytic philosophy.
...
- **Unified Knowledge Base**: All philosophical texts from 82+ authors (~64,236 chunks across 236 unique works) are consolidated into a single "Common Fund" with `figureId='common'`. Author filtering is implemented with ILIKE partial matching and robust normalization for 38+ philosopher names.
