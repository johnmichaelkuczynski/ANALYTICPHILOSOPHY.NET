...
**Authentication: NONE.** The login system was ripped out entirely per user directive (July 2026). There is no OAuth, no sessions, no `express-session`/`connect-pg-simple`, no auth middleware, and no route gating anywhere — every page and every `/api/*` endpoint is open in both development and production. The landing page has plain "Start the course" / "Enter the course" buttons that route straight to the dashboard. The `users` and `login_events` tables and the `/logins` + `/admin/visitors` endpoints remain (harmless, empty) so the Administrative page still renders. **Do not re-add any authentication unless the user explicitly asks for it.**
