...
export function setupAuth(app: Express) {
  ...
  const sessionDbIsLocal = /@(localhost|127\.0\.0\.1)[:/]/.test(
    sessionConnectionString || ""
  );
  const pool = new pg.Pool({
    connectionString: sessionConnectionString,
    ...(sessionDbIsLocal ? {} : { ssl: { rejectUnauthorized: false } }),
  });
  ...
  if (googleEnabled) {
    // Callback path is /api/auth/google/callback: this app's reverse proxy
    // only routes /api-prefixed paths to the API server, so the callback must
    // live under /api to be reachable.
    ...
    const getCallbackURL = () => {
      if (process.env.NODE_ENV === "production") {
        const prodDomain = (process.env.REPLIT_DOMAINS || "")
          .split(",")[0]
          ?.trim();
        return `https://${prodDomain || process.env.REPLIT_DEV_DOMAIN || "localhost"}${CALLBACK_PATH}`;
      }
      if (process.env.REPLIT_DEV_DOMAIN) {
        return `https://${process.env.REPLIT_DEV_DOMAIN}${CALLBACK_PATH}`;
      }
      return `http://localhost:${process.env.PORT || 8080}${CALLBACK_PATH}`;
    };
