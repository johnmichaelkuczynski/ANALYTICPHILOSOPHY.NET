...
export function Scene5({ setCursorPos, setIsClicking }: { setCursorPos: (pos: {x: string, y: string}) => void, setIsClicking: (val: boolean) => void }) {
  ...
  const [activeTab, setActiveTab] = useState("Quantifiers");
  ...
  useEffect(() => {
    ...
    const t12 = setTimeout(() => setTypedAnswer("∃x(Sx)"), 9000);
    const t12a = setTimeout(() => setTypedAnswer("∃x(Sx) "), 9300);
    ...
    const t16 = setTimeout(() => setTypedAnswer("∃x(Sx) ∧ ¬Ss"), 10200);
    ...
    >
      ...
      <div className="max-w-4xl mx-auto w-full">
        ...
        <div className="flex justify-between items-end mb-8">
          <div>
            <h1 className="text-4xl font-serif text-primary mb-2">Quantificational logic: ∀ and ∃</h1>
        ...
        <div className="bg-white border border-border rounded-xl shadow-sm overflow-hidden flex flex-col min-h-[400px]">
          <div className="p-8 flex-1 border-b border-border text-lg text-primary font-medium leading-relaxed">
            {phase >= 1 && phase < 4 && (
              <StreamingText text="Write the logical form of 'Someone smokes'. Use S(x) for 'x smokes'." delay={0} />
            )}
            {phase >= 4 && (
              <StreamingText text="Now try this: 'Someone smokes but Smith does not'. Use s for Smith." delay={0} />
            )}
          </div>
          ...
          <div className="p-6 bg-muted/20">
              ...
              {phase >= 2 && phase < 4 && (
                ...
                >
                  ...
                  <div className="text-sm text-emerald-900 leading-relaxed">
                    <StreamingText text="Correct. ∃x(Sx) means there is at least one entity x such that x smokes. Existence is instantiation." delay={0} />
                  </div>
                </motion.div>
              )}
              {phase >= 5 && (
                ...
                >
                  ...
                  <div className="text-sm text-emerald-900 leading-relaxed">
                    <StreamingText text="Spot on. The 'but' is translated simply as a conjunction (AND). ∃x(Sx) ∧ ¬Ss." delay={0} />
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <div className="mt-4 border rounded-md p-3 bg-secondary/50">
              <div className="flex space-x-2 pb-2 mb-3 overflow-x-auto text-xs font-medium">
                {['Connectives', 'Quantifiers', 'Modal', 'Turnstiles'].map(t => (
